...
/*$log  = "Log time: ".date("F j, Y, g:i a").PHP_EOL.
	"Message : $_REQUEST : ".var_export($_REQUEST,true).PHP_EOL.
	"-------------------------";
//Save string to log, use FILE_APPEND to append.
file_put_contents('./logs/log_confirmOf_'.date("j.n.Y").'.txt', $log, FILE_APPEND);
*/
//Modified By DB
//Don't think it is useful and causes etat=attente to die here
//$test_etat=false;
...
/*file_put_contents(
	'./logs/log_confirmOf_'.date("j.n.Y").'.txt',
	"\$test_etat : \n"
	.var_export($test_etat,true)."\n",
	FILE_APPEND);
*/
//if($test_etat) {
	 ...
	 *
	 */
	if((($payment->get_bouticId() !== Tools::getValue('id_boutique')) ||
	!$payment->get_bouticId() ||
	!Tools::getValue('id_boutique') ||
	Tools::getValue('id_boutique') == '')) {
		
		//die('INVALID MERCHANT ID '.Tools::getValue('id_boutique').' FOR THIS BOUTIC !');
		$feedback.= 'Attention : INVALID MERCHANT ID '.Tools::getValue('id_boutique').' FOR THIS BOUTIC !';
	}
				...
				switch ($old_state) {
					...
					case Configuration::get('PS_OS_CANCELED'):
						if (Tools::getValue('etat') == 'ok') {
							...
							if (Tools::getValue('num_abo') &&
							Tools::getValue('num_abo') > 0) {
