...
class Bluepaid extends PaymentModule
{
	...
	/*
	 * DEFINE BLUEPAID SERVER NAME
	 * Convert domain name into IP address
	 */
 	private $bpi_default_ip = array(0=>'193.33.47.34', 1=>'193.33.47.35', 2=>'193.33.47.39', 3=>'87.98.218.80' );
	private $bpi_default_domain = array(0=>'bluepaid.com', 1=>'test-paiement-securise.bluepaid.com', 2=>'paiement-securise.bluepaid.com', 3=>'monitoring.bluepaid.com' );
	...
	public function __construct()
	{
		...
		$this->bootstrap = true;
		$this->module_key = '5a7a32bbd36150fd065eb781a7bfc8c6';
		
		parent::__construct();

		$this->author = 'Bluepaid';
	...
	*/
	...
	public function uninstall() {
		...
		Configuration::deleteByName('BPI_AUTHORIZED_DOMAIN');
	...
	*/
	...
	public function getContent() {
		...
		if (isset($_POST['submitBluepaid_config'])) {
			...
			if (isset($_POST['bpi_authorized_domain']))
				Configuration::updateValue('BPI_AUTHORIZED_DOMAIN', $_POST['bpi_authorized_domain']);
		...
		$bpi_authorized_domain = 'bluepaid.com;monitoring.bluepaid.com;test-paiement-securise.bluepaid.com;paiement-securise.bluepaid.com';
		
		
		$base_url = __PS_BASE_URI__;
		$conf_uri = htmlspecialchars($_SERVER['HTTP_HOST'], ENT_COMPAT, 'UTF-8').$base_url.'modules/bluepaid/confirmOf.php';
		$bp_img = '//'.htmlspecialchars($_SERVER['HTTP_HOST'], ENT_COMPAT, 'UTF-8').$base_url.'modules/bluepaid/';
		
		$context->smarty->assign(array(
			'legend' => $this->l('Configure your bluepaid module'),
			'merchid' => $merchid,
			'bluepaid_form' => $_SERVER['REQUEST_URI'],
			'bluepaid_confirmation' => $ht,
			...
			'bpi_authorized_domain' > $bpi_authorized_domain,
		...
		if (version_compare(_PS_VERSION_, '1.6', '<')) {
			...
			//$context->controller->addCSS($this->_path.'views/css/bluepaid.css', 'all');
	...
	public function hookPayment($params) {
		...
		$context->smarty->assign('bp_test', 'test');

		/* Added for PrestaBox */
		if (method_exists($this->context->controller, 'addCSS'))
			$this->context->controller->addCSS(_MODULE_DIR_.$this->name.'/views/css/bluepaid.css');
		else
			Tools::addCSS(_MODULE_DIR_.$this->name.'/views/css/bluepaid.css');
		
		
		if($this->_authorize_Xpayment() && 
		($total_cart>=$this->_getMinAmount())) {	
			...
			$context->smarty->assign(array(
				'bluepaid_multipayment' => false,
				'bluepaid_multipayment_nbmax' => 0,
			));	
		}
	...
	public function Is_authorizedDomain($domain='')
	{
		if($domain)
		{
			$a_domainaddressbpi=Configuration::get('BPI_AUTHORIZED_DOMAIN');
			if($a_domainaddressbpi)
				$domainaddressbpi=explode(';', $a_domainaddressbpi);
			else
				$domainaddressbpi = $this->bpi_default_domain;

			if($domainaddressbpi) {

				if(is_array($domainaddressbpi)) {

					foreach($domainaddressbpi as $key=>$value) {

						if($value == $domain){
							return true;
						}
					}
				} elseif($domainaddressbpi==$domain) {
	...
	*/
	...
	public function savePayment($order, $bluepaid_response) {
		...
		if (!$this->isSuccessState($order) &&
		!$bluepaid_response['etat'] != 'ok')
			// no payment creation
			return;
